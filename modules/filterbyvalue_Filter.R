createFilterbyvalueUI <- function(id, suffix="") {
    ...
    filterByValueUI <- list(
        column(
            6,
            h3(paste0("Filter by values ")), # Ajouter le suffixe au titre
            sliderInput(
                if(suffix == "_corr"){
                     inputId = ns(paste0("slider_minmax_x", suffix)) # Ajouter le suffixe à inputId
                } else {
                   inputId = ns(paste0("slider_minmax", suffix)) # Ajouter le suffixe à inputId
                },
            ...
            if (suffix == "_corr") {
                sliderInput(
                    inputId = ns(paste0("slider_minmax_y", suffix)), # Ajouter le suffixe à inputId
                    label = h6("Filter values of y-axis variable (slider shows range of values in y-axis variable)"),
                    min = 0, max = 100, value = c(0, 100)
                )
            },
            checkboxGroupInput(
                inputId = ns(paste0("selected_quarts", suffix)), # Ajouter le suffixe à inputId
